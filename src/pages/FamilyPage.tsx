...
const FamilyPage = () => {
  ...
  const handleDeleteContact = (c: typeof contacts[0]) => {
    if (!canEdit(c.added_by)) { alert("You can only edit items you created"); return; }
    deleteContact(c.id);
  };
  ...
  return (
    <div className="max-w-lg mx-auto px-4 pt-4 pb-36">
                ...
                {currentUser.role === 'patient' && (
                  <div className="mt-3 pt-3 border-t border-border">
                    <button onClick={() => setEditingPerms(editingPerms === fm.id ? null : fm.id)}
                      className="text-sm text-lavender flex items-center gap-1">
                        ...
                        {(['can_view', 'can_add', 'can_edit'] as const).map(perm => (
                          <label key={perm} className="flex items-center justify-between">
                            ...
                            <button
                              ...
                              className={`w-12 h-6 rounded-full transition-colors duration-300 ${fm[perm] ? 'bg-mint' : 'bg-muted'}`}
                            >
                              <div className={`w-5 h-5 rounded-full bg-card shadow transition-transform duration-300 ${fm[perm] ? 'translate-x-6' : 'translate-x-0.5'}`} />
                            </button>
                          </label>
                        ))}
                      </div>
      ...
      <div className="flex items-center justify-between mb-3">
        ...
        <button onClick={() => setShowAddContact(!showAddContact)} className="p-2 rounded-full bg-lavender/20">
      ...
      {showAddContact && (
        <GlassCard className="p-4 mb-4 animate-scale-in">
          <div className="space-y-3">
            ...
            <button onClick={handleAddContact} className="btn-pink w-full">Save Contact</button>
          </div>
        </GlassCard>
      )}
      ...
      <div className="space-y-2 mb-6">
        {contacts.map(c => (
          <GlassCard key={c.id} className="p-3 flex items-center justify-between">
            ...
            {canEdit(c.added_by) && (
              <button onClick={() => handleDeleteContact(c)} className="p-2 text-destructive/60 hover:text-destructive">
                <Trash2 size={16} />
              </button>
            )}
          </GlassCard>
      ...
      <div className="mb-4">
        <div className="flex items-center justify-between mb-3">
          ...
          <button onClick={() => navigate('/audit')} className="text-sm text-lavender">View All</button>
        </div>
        <div className="space-y-2">
