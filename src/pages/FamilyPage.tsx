...
const FamilyPage = () => {
  ...
  const handleAddContact = () => {
    ...
    addContact({
      ...
      relationship: relationship.trim(), is_emergency: isEmergency, added_by: currentUser.id,
    });
    setName(''); setPhone(''); setRelationship(''); setIsEmergency(false); setShowAddContact(false);
  };
  ...
  const togglePermission = (fm: typeof familyMembers[0], perm: 'can_view' | 'can_add' | 'can_edit') => {
    ...
    updateFamilyMember(fm.id, { [perm]: !fm[perm] });
  };
  ...
  return (
    <div className="max-w-lg mx-auto px-4 pt-12 pb-36">
      <h1 className="text-2xl font-bold mb-4">Family & Friends</h1>
      ...
      <div className="grid grid-cols-3 gap-3 mb-6">
        ...
        <GlassCard className="p-3 text-center">
          <p className="text-2xl font-bold text-sky-blue">{contacts.filter(c => c.is_emergency).length}</p>
          <p className="text-xs text-muted-foreground">SOS Contacts</p>
        </GlassCard>
        <GlassCard className="p-3 text-center">
                        ...
                        <div className="mt-2 space-y-2 animate-fade-in">
                          {(['can_view', 'can_add', 'can_edit'] as const).map(perm => (
                            <label key={perm} className="flex items-center justify-between min-h-[44px]">
                              ...
                              <button onClick={() => togglePermission(fm, perm)}
      ...
      {showAddContact && (
        <GlassCard className="p-4 mb-4 animate-scale-in">
          <div className="space-y-3">
            ...
            <input value={relationship} onChange={e => setRelationship(e.target.value)} placeholder="Relationship (e.g. Daughter)" className="input-glass w-full" />
            <label className="flex items-center gap-2 cursor-pointer min-h-[48px]">
              <input type="checkbox" checked={isEmergency} onChange={e => setIsEmergency(e.target.checked)} className="w-5 h-5 rounded accent-destructive" />
              <span className="text-sm">Emergency Contact (SOS)</span>
      ...
      <div className="space-y-2 mb-6">
        {contacts.length === 0 ? (
          <GlassCard className="p-4 text-center"><p className="text-muted-foreground text-sm">No contacts yet. Add your first one above!</p></GlassCard>
        ) : contacts.map(c => (
          <GlassCard key={c.id} className="p-3 flex items-center justify-between">
            ...
            {canEdit(c.added_by) && (
              <button onClick={() => handleDeleteContact(c)} className={`p-2 min-w-[44px] min-h-[44px] flex items-center justify-center ${confirmDelete === c.id ? 'text-destructive' : 'text-destructive/60 hover:text-destructive'}`}>
